...
int PerfSymbolTable::insertSubprogram(Dwarf_Die *top, Dwarf_Addr entry, quint64 offset, quint64 size, qint32 binaryId, qint32 binaryPathId,
                                      qint32 inlineCallLocationId, bool isKernel)
{
    ...
    m_unwind->resolveSymbol(locationId, PerfUnwind::Symbol(symId, mangleId, offset, size, binaryId, binaryPathId, isKernel));

    return locationId;
}

int PerfSymbolTable::parseDie(Dwarf_Die *top, quint64 offset, quint64 size, qint32 binaryId, qint32 binaryPathId, bool isKernel,
                              Dwarf_Files *files, Dwarf_Addr entry, qint32 parentLocationId)
{
    ...
    case DW_TAG_inlined_subroutine: {
        ...
        return insertSubprogram(top, entry, offset, size, binaryId, binaryPathId, callLocationId, isKernel);
    }
    case DW_TAG_subprogram:
        return insertSubprogram(top, entry, offset, size, binaryId, binaryPathId, -1, isKernel);
    default:
        return -1;
    }
}

qint32 PerfSymbolTable::parseDwarf(Dwarf_Die *cudie, Dwarf_Die *subroutine, Dwarf_Addr bias, quint64 offset, quint64 size, qint32 binaryId,
                                   qint32 binaryPathId, bool isKernel)
{
    ...
    for (int i = nscopes - 1; i >= 0; --i) {
        ...
        auto locationId = parseDie(scope, offset, size, binaryId, binaryPathId, isKernel, files, scopeAddr, parentLocationId);
        if (locationId != -1)
    ...
    if (mod) {
        ...
        if (off == addressLocation.address) {// no symbol found
            ...
            if (subroutine && !m_unwind->hasSymbol(addressLocation.parentLocationId))
                functionLocation.parentLocationId = parseDwarf(die, subroutine, bias, offset, size, binaryId, binaryPathId, isKernel);
        ...
        if (!m_unwind->hasSymbol(addressLocation.parentLocationId)) {
            ...
            qint32 mangleId = m_unwind->resolveString(symname);
            qint32 symId = m_unwind->resolveString(demangle(symname));
            m_unwind->resolveSymbol(addressLocation.parentLocationId,
                                    PerfUnwind::Symbol(symId, mangleId, offset, size, binaryId, binaryPathId, isKernel));
        }
    } else {
        if (isKernel) {
